...
function renderProduction(){const s=statuses();$("productionBoard").innerHTML=s.map(st=>`<section class="kanban-col"><h3>${st} (${db.orders.filter(x=>x.status===st).length})</h3>${db.orders.filter(x=>x.status===st).map(x=>`<article class="kanban-card"><strong>${esc(x.number)} — ${esc(x.customer||"Mijoz")}</strong><p>${money(x.total)} · ${x.deadline||""}</p><select data-prod="${x.id}">${s.map(a=>`<option ${a===x.status?"selected":""}>${a}</option>`).join("")}</select></article>`).join("")}</section>`).join("")}
